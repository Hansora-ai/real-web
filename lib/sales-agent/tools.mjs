...
const { MODELS, FEATURES, CREDIT_DISPLAY_MULTIPLIER, normalizeModelId, quoteModel, getPackages, toDisplayedCredits } = catalog;
...
export const TOOL_DEFINITIONS = [
  {
    type: 'function', name: 'get_available_features', strict: true,
    description: 'List verified Hansora website functions and creative tools other than AI models. Use when asked what Hansora can do, what features/functions/tools it has, or what it offers besides models. Do not substitute the sales agent\'s own capabilities.',
...
export async function executeTool(name, args, { user } = {}) {
  if (name === 'get_available_features') {
    return {
      ok: true,
      features: Object.values(FEATURES).map(({ id, name: featureName, category, status, description }) => ({
        id, name: featureName, category, status, description
      }))
    };
  }
