  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (body) {
        ...
        else reject(new Error('Unsupported body type'));
      }
      req.end();
    } catch (e) {
      reject(e);
...
exports.handler = async (event) => {
  try {
    ...
    for (const img of images) {
      const res = await uploadToSupabase({ SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SUPABASE_BUCKET, SIGN_EXP }, img.data, img.mime, img.filename);
