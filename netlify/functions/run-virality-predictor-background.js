...
exports.handler = async (event) => {
  ...
  try {
    ...
    generationId = await saveProcessingGeneration({ uid, runId, fileName, videoUrl, duration, language });
    ...
    try {
      refunded = debited && uid ? await updateCredits(uid, COST) : false;
    } catch {}
    if (uid && runId) {
      try {
    ...
    return json(200, {
      ...
      error: messageOf(error) || "analysis_failed",
      refunded,
      refund_amount: refunded ? COST : 0,
      failed_state_saved: failedStateSaved
    });
  }
};
...
async function saveProcessingGeneration({ uid, runId, fileName, videoUrl, duration, language }) {
  ...
  const existing = await fetch(`${UG_URL}?select=id&user_id=eq.${encodeURIComponent(uid)}&kind=eq.virality_predictor&meta->>run_id=eq.${encodeURIComponent(runId)}&order=created_at.desc&limit=1`, { headers: sb() });
  if (!existing.ok) throw new Error(`supabase_find_${existing.status}`);
  ...
  const created = await fetch(UG_URL, {
    method: "POST",
    headers: { ...sb(), "Content-Type": "application/json", Prefer: "return=representation" },
    body: JSON.stringify(payload)
  });
  if (!created.ok) throw new Error(`supabase_insert_${created.status}`);
  const createdRows = await created.json().catch(() => []);
  return Array.isArray(createdRows) && createdRows[0] && createdRows[0].id ? String(createdRows[0].id) : "";
}

async function patchGenerationDone({ uid, runId, fileName, videoUrl, duration, language, generationId, analysis, raw }) {
  if (!UG_URL || !SERVICE_KEY) return;
  const meta = {
    run_id: runId,
    status: "done",
  ...
  const query = generationId
    ...
    : `?user_id=eq.${encodeURIComponent(uid)}&kind=eq.virality_predictor&meta->>run_id=eq.${encodeURIComponent(runId)}`;
  const res = await fetch(`${UG_URL}${query}`, {
    method: "PATCH",
    headers: { ...sb(), "Content-Type": "application/json", Prefer: "return=minimal" },
    body: JSON.stringify({ result_url: videoUrl, meta })
  });
  if (!res.ok) throw new Error(`supabase_done_${res.status}`);
}

async function patchGenerationFailed({ uid, runId, fileName, videoUrl, duration, language, generationId, error, refunded }) {
  if (!UG_URL || !SERVICE_KEY) return;
  const meta = {
    run_id: runId,
    status: "failed",
  ...
  const query = generationId
    ? `?id=eq.${encodeURIComponent(generationId)}`
    : `?user_id=eq.${encodeURIComponent(uid)}&kind=eq.virality_predictor&meta->>run_id=eq.${encodeURIComponent(runId)}`;
  const patched = await fetch(`${UG_URL}${query}`, {
    method: "PATCH",
    headers: { ...sb(), "Content-Type": "application/json", Prefer: "return=representation" },
    body: JSON.stringify({ result_url: null, meta })
  });
  const patchedRows = patched.ok ? await patched.json().catch(() => []) : [];
  if (patched.ok && Array.isArray(patchedRows) && patchedRows.length) return;

  const fallback = await fetch(UG_URL, {
    method: "POST",
    headers: { ...sb(), "Content-Type": "application/json", Prefer: "return=minimal" },
    body: JSON.stringify({
      user_id: uid,
  ...
  if (!fallback.ok) throw new Error(`supabase_failed_${fallback.status}`);
}
