// netlify/functions/run-grok-image.js
// Grok Image launcher via KIE createTask with server-side credit debit (idempotent per run_id).
// - Text to Image: model grok-imagine/text-to-image
// - Image to Image: model grok-imagine/image-to-image
...
const VERSION_TAG  = "grok_image_kie_v1";
...
async function seedUserGeneration(uid, run_id, prompt, provider, metaExtra){
  ...
  try{
    const ug = `${SUPABASE_URL}/rest/v1/user_generations`;
    const meta = Object.assign({ source:'grok-image', run_id, model:'grok-image', status:'pending' }, (metaExtra||{}));
...
exports.handler = async (event) => {
  ...
  try{
    ...
    if (isImageToImage) input.image_urls = input_urls;
      ...
      try {
        if (SUPABASE_URL && SERVICE_KEY && row_id) {
          await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
            ...
            body: JSON.stringify({ meta: { source:"grok-image", run_id, model, status:"create_failed", task_id: id, raw: js } })
          });
        }
      } catch {}
    ...
    try {
      if (SUPABASE_URL && SERVICE_KEY && row_id) {
        await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
          ...
          body: JSON.stringify({ meta: { source:"grok-image", run_id, model, status:"processing", task_id: id, aspect_ratio, resolution:"720p", mode: isImageToImage ? "image-to-image" : "text-to-image" } })
        });
      }
    } catch {}

    // Debit credits AFTER provider accepted and exactly once per (uid, run_id)
    const baseMeta = { source:"grok-image", run_id, model, status:"processing", task_id: id, aspect_ratio, resolution:"720p", mode: isImageToImage ? "image-to-image" : "text-to-image" };
