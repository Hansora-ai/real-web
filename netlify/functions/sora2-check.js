// netlify/functions/sora2-check.js (KIE Market)
const VERSION = "sora2-check-GET-2025-10-06+v4-kie-market";
...
exports.handler = async (event) => {
  ...
  const url = `${KIE_BASE}/api/v1/market/record-info?taskId=${encodeURIComponent(taskId)}`;
  ...
  let video_url = "";
  for (const u of urls){ if (/^https?:\/\//i.test(u) && /\.mp4(\?|#|$)/i.test(u)) { video_url = u; break; } }
  ...
  try {
    if (SUPABASE_URL && SERVICE_KEY) {
      ...
      if (idToPatch) {
        await fetch(`${UG_URL}?id=eq.${encodeURIComponent(idToPatch)}`, { method:"PATCH", headers:{ ...sb(), "Content-Type":"application/json", "Prefer":"return=minimal" }, body: JSON.stringify(payload) });
      } else {
        await fetch(UG_URL, { method:"POST", headers:{ ...sb(), "Content-Type":"application/json", "Prefer":"return=minimal" }, body: JSON.stringify({ user_id: uid || "00000000-0000-0000-0000-000000000000", provider:"sora2", kind:"video", prompt:null, result_url: video_url, meta:{ run_id, task_id: taskId, status:"done" } }) });
      }
    }
  } catch {}
  ...
  try {
    if (TABLE_URL) {
      await fetch(TABLE_URL, { method:"POST", headers:{ ...sb(), "Content-Type":"application/json", "Prefer":"resolution=merge-duplicates,return=minimal" }, body: JSON.stringify([{ user_id: uid || "00000000-0000-0000-0000-000000000000", run_id, task_id: taskId, image_url: video_url }]) });
    }
  } catch {}

  if (debug) out.debug = { provider_status: r.status };
  return json(200, out);
};
