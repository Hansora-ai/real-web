...
// Webhook receiver for Higgsfield (1:1 logic style to Veo callback).
// Validates secret (optional), extracts result URLs, writes into Supabase.

const { URL } = require("url");
const https = require("https");

const HF_WEBHOOK_SECRET = process.env.HF_WEBHOOK_SECRET || ""; // must match run-higgsfield.js
...
exports.handler = async (event) => {
  try{
    ...
    // optional signature/secret check (if provider sends it back)
    ...
    // Prefer run_id; fallback to job_set_id
    let updated = false;
    if (run_id) {
