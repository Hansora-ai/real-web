// netlify/functions/run-seedream-5-pro.js
// Seedream 5.0 Pro launcher with server-side credit debit (Service Role) + run_id idempotency.
...
const VERSION_TAG  = "seedream_5_pro_fn_v1";
...
function normalizeImageSize(v) {
  ...
  const direct = new Set(["1:1","2:3","3:2","3:4","4:3","9:16","16:9"]);
...
function normalizeQuality(v, resolution) {
  const quality = String(v || "").trim().toLowerCase();
  if (quality === "high" || quality === "basic") return quality;
  return String(resolution || "").trim().toLowerCase() === "2k" ? "high" : "basic";
}

function costForQuality(quality) {
  return quality === "high" ? 1 : 0.5;
}
...
async function seedUserGeneration(uid, run_id, prompt, metaExtra){
  ...
  try{
    ...
    const meta = Object.assign(
      { source:"seedream-5-pro", run_id, model:"seedream-5-pro", status:"pending", charged:"false" },
      (metaExtra||{})
    );
...
exports.handler = async (event) => {
  ...
  try{
    ...
    const quality = normalizeQuality(body.quality, body.resolution);
    ...
    const payload = {
      model: hasImages ? "seedream/5-pro-image-to-image" : "seedream/5-pro-text-to-image",
      input,
    ...
    const baseMeta = { run_id, task_id: taskId, size, quality, status:"processing", refund_amount: cost };
    const seeded = await seedUserGeneration(uid, run_id, prompt, baseMeta);
    const row_id = seeded?.row_id || null;

    const charged = await chargeOnceForRun(uid, run_id, cost, row_id, { ...baseMeta, source:"seedream-5-pro", model:"seedream-5-pro" });
