// netlify/functions/run-video-relight-gemini-omni.js
// Launches Video Relight jobs through KIE using the original video plus start/end timing.
const KIE_BASE = (process.env.KIE_BASE_URL || 'https://api.kie.ai').replace(/\/+$/, '');
const KIE_KEY = process.env.KIE_API_KEY || '';
const SUPABASE_URL = (process.env.SUPABASE_URL || '').replace(/\/+$/, '');
const SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
const SITE_BASE = (process.env.SITE_BASE || 'https://hansora.co').replace(/\/+$/, '');
const CALLBACK_BASE = `${SITE_BASE}/.netlify/functions/video-relight-check`;
...
function buildProtectedRelightPrompt(body) {
  const rawPrompt = String(body.prompt || '').trim();
  const preset = String(body.lighting_preset || '').trim();
  const presetPrompt = String(body.lighting_prompt || '').trim();
  const userNote = String(body.user_lighting_note || '').trim();
  const lightingReferenceCount = Array.isArray(body.image_urls) ? body.image_urls.filter(Boolean).length : 0;
  const lightingText = [preset ? `Selected lighting preset: ${preset}.` : '', presetPrompt, userNote, rawPrompt]
    .filter(Boolean)
    .join(' ');
  const lightingReferenceInstruction = lightingReferenceCount
    ? `Use the uploaded lighting reference image${lightingReferenceCount === 1 ? '' : 's'} only as lighting examples: match their light direction, color temperature, exposure mood, contrast, shadow softness, highlight behavior, reflections, and atmosphere. Do not copy their subjects, objects, background, composition, identity, clothing, location, or scene content.`
    : 'No lighting reference image was provided; use only the selected lighting preset and written lighting instruction.';
  return [
    'Relight the uploaded video only.',
    'Keep every subject, identity, face, body, clothing, object, background, camera angle, camera motion, framing, timing, action, pose, expression, composition, and scene content unchanged.',
    'Do not replace people, do not change the environment, do not add or remove objects, do not change the story, and do not stylize the footage.',
    lightingText ? `Lighting instruction: ${lightingText}` : 'Lighting instruction: improve the lighting with realistic cinematic relighting.',
    lightingReferenceInstruction,
    'Apply physically consistent light direction, shadow behavior, reflections, exposure, color temperature, contrast, and highlight rolloff across the selected clip.',
    'The only visible change should be lighting, color mood, illumination, shadows, and highlights.'
  ].join(' ');
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const prompt = buildProtectedRelightPrompt(body);
    ...
    const model = String(process.env.VIDEO_RELIGHT_GEMINI_MODEL || process.env.VIDEO_EDIT_GEMINI_MODEL || process.env.GEMINI_OMNI_VIDEO_MODEL || 'gemini-omni-video').trim();
    const metaBase = {
      source: 'kie',
      engine: 'video-relight-gemini-omni',
      source_feature: 'video-relight',
      run_id: runId,
      status: 'pending',
      refund_amount: cost,
      ...
      lighting_reference_count: imageUrls.length,
      lighting_preset: String(body.lighting_preset || '').trim(),
      user_lighting_note: String(body.user_lighting_note || '').trim(),
    };

    const rowId = existing?.id || await insertGeneration(uid, prompt, metaBase);
