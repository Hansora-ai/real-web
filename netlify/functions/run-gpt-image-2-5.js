// netlify/functions/run-gpt-image-2-5.js
// GPT Image 2.5 Sunburst launcher via KIE createTask with server-side credit debit (idempotent per run_id).
// - Text to Image: model gpt-image-2-5-sunburst-text-to-image
// - Image to Image: model gpt-image-2-5-sunburst-image-to-image
...
const VERSION_TAG  = "gpt_image_2_5_sunburst_kie_v1";
...
function normalizeAspectRatio(v) {
  ...
  const direct = new Set(["auto","1:1","3:2","2:3","4:3","3:4","16:9","9:16","21:9","27:16","16:27","9:8","8:9","5:4","4:5","2:1","1:2","3:1","1:3","9:21"]);
...
function normalizeResolution(v) {
  let resolution = String(v || "1K").trim().toUpperCase();
...
async function seedUserGeneration(uid, run_id, prompt, provider, metaExtra){
  ...
  try{
    const ug = `${SUPABASE_URL}/rest/v1/user_generations`;
    const meta = Object.assign({ source:'gpt-image-2-5', run_id, model:'gpt-image-2-5', status:'pending' }, (metaExtra||{}));
...
async function hasUnlimitedSubscription(uid, modelKey, resolution){
  ...
  try{
    ...
    if (modelKey === "gpt-image-2-5") {
      if ((row.plan_id === "premium_monthly" || row.plan_id === "pro_monthly") && res === "1K") return true;
      if (row.plan_id === "pro_max_monthly" && (res === "1K" || res === "2K")) return true;
    }
    return false;
  }catch(_e){ return false; }
}
...
exports.handler = async (event) => {
  ...
  try{
    ...
    const input_urls = urls.map(u => String(u)).filter(Boolean).slice(0, 16);

    const isImageToImage = input_urls.length > 0;
    const model = isImageToImage ? "gpt-image-2-5-sunburst-image-to-image" : "gpt-image-2-5-sunburst-text-to-image";
    ...
    const subscriptionUnlimited = String(body.billing_mode || "").toLowerCase() === "unlimited"
      && queueAuthorized && await hasUnlimitedSubscription(uid, "gpt-image-2-5", resolution);
    const chargeCost = subscriptionUnlimited ? 0 : cost;

    // Provider label: keep stable, include mode
    const provider = "GPT Image 2.5";
      ...
      try {
        if (SUPABASE_URL && SERVICE_KEY && row_id) {
          await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
            ...
            body: JSON.stringify({ meta: { source:"gpt-image-2-5", run_id, model, status:"create_failed", task_id: id, raw: js, refund_amount: 0, charge_cost: chargeCost, subscription_unlimited: subscriptionUnlimited } })
          });
        }
      } catch {}
    ...
    try {
      if (SUPABASE_URL && SERVICE_KEY && row_id) {
        await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
          ...
          body: JSON.stringify({ meta: { source:"gpt-image-2-5", run_id, model, status:"processing", task_id: id, aspect_ratio, resolution, mode: isImageToImage ? "image-to-image" : "text-to-image", refund_amount: 0, charge_cost: chargeCost, subscription_unlimited: subscriptionUnlimited } })
        });
      }
    } catch {}

    // Debit credits AFTER provider accepted and exactly once per (uid, run_id)
    const baseMeta = { source:"gpt-image-2-5", run_id, model, status:"processing", task_id: id, aspect_ratio, resolution, mode: isImageToImage ? "image-to-image" : "text-to-image", refund_amount: 0, charge_cost: chargeCost, subscription_unlimited: subscriptionUnlimited };
