// netlify/functions/run-video-edit-gemini-omni.js
// Launches Video Edit jobs through KIE using the original video plus start/end timing.
const KIE_BASE = (process.env.KIE_BASE_URL || 'https://api.kie.ai').replace(/\/+$/, '');
const KIE_KEY = process.env.KIE_API_KEY || '';
const SUPABASE_URL = (process.env.SUPABASE_URL || '').replace(/\/+$/, '');
const SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
const SITE_BASE = (process.env.SITE_BASE || 'https://hansora.co').replace(/\/+$/, '');
const CALLBACK_BASE = `${SITE_BASE}/.netlify/functions/video-edit-check`;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const model = String(process.env.VIDEO_EDIT_GEMINI_MODEL || process.env.GEMINI_OMNI_VIDEO_MODEL || 'gemini-omni-video').trim();
    const metaBase = {
      source: 'kie',
      engine: 'video-edit-gemini-omni',
