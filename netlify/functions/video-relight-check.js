// netlify/functions/video-relight-check.js
// Dedicated checker/refunder for Video Relight jobs.
...
exports.handler = async (event) => {
  try {
    ...
    const row = await findVideoRelightGeneration(ids);
...
function isVideoRelightRow(row) {
  const meta = row?.meta && typeof row.meta === 'object' ? row.meta : {};
  const provider = String(row?.provider || '').toLowerCase();
  const engine = String(meta.engine || '').toLowerCase();
  const feature = String(meta.source_feature || '').toLowerCase();
  return feature === 'video-relight' || engine === 'video-relight-gemini-omni' || provider === 'video relight';
}

async function findVideoRelightGeneration(ids) {
  ...
  for (const query of queries) {
    ...
    if (!row || row.result_url || !isVideoRelightRow(row)) continue;
