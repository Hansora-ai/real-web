// netlify/functions/vv-check.js
// GET poller for Veo 3 (KIE) tasks. Finds mp4 and backfills Supabase.
// Mirrors rv-check.js but points to the Veo "record-detail" endpoint.

const VERSION_TAG = "vv-check-GET-2025-09-28+v1";
      ...
      if (SUPABASE_URL && SERVICE_KEY) {
        ...
        if (idToPatch) {
          ...
          const ir = await fetch(UG_URL, {
            method: "POST",
            headers: { ...sb(), "Content-Type": "application/json", "Prefer": "return=minimal" },
            body: JSON.stringify({ user_id: uid || "00000000-0000-0000-0000-000000000000", provider:"veo3", kind:"video", prompt:null, result_url: video_url, meta: { run_id, task_id: taskId, status: "done" } })
          });
          patched = ir.ok;
