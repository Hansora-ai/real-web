// netlify/functions/mjv-check.js
// GET poller for Midjourney (KIE jobs) tasks. Finds mp4 and backfills Supabase.
// Mirrors vv-check.js style but uses /jobs/getTaskResult.

const VERSION_TAG = "mjv-check-GET-2025-09-29+v1";
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const url = `${KIE_BASE}/api/v1/jobs/getTaskResult?taskId=${encodeURIComponent(taskId)}`;
      ...
      if (SUPABASE_URL && SERVICE_KEY) {
        ...
        if (idToPatch) {
          ...
          const ir = await fetch(UG_URL, {
            method: "POST",
            headers: { ...sb(), "Content-Type": "application/json", "Prefer": "return=minimal" },
            body: JSON.stringify({ user_id: uid || "00000000-0000-0000-0000-000000000000", provider:"midjourney", kind:"video", prompt:null, result_url: video_url, meta: { run_id, task_id: taskId, status: "done" } })
          });
          patched = ir.ok;
