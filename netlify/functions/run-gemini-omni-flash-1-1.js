...
// Launches KIE Gemini Omni 1.1 Flash jobs using the website Gemini Omni workflow.
...
function normalizeResolution(value) {
  ...
  return ['720p', '1080p', '4k'].includes(key) ? key : '1080p';
}
...
function costFor(body) {
  ...
  const table = resolution === '4k'
    ? { 4: 10, 6: 11, 8: 12, 10: 14 }
    : { 4: 4, 6: 5.5, 8: 6.9, 10: 8.3 };
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (videoUrl && !/^https:\/\//i.test(videoUrl)) return json(400, { ok: false, error: 'invalid_video_url' });
    ...
    if (videoUrl && (videoWindow.ends <= videoWindow.start || videoWindow.ends - videoWindow.start > 10.0001)) {
      return json(400, { ok: false, error: 'invalid_video_window', message: 'Choose no more than 10 seconds from the source video.' });
    }
    const model = String(process.env.GEMINI_OMNI_FLASH_1_1_MODEL || 'google/gemini-omni-flash-1-1').trim();
    const metaBase = {
      source: 'kie',
      engine: 'gemini-omni-flash-1-1',
      run_id: runId,
      status: 'pending',
      refund_amount: cost,
    };
