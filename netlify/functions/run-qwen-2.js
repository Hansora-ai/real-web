...
function aspectToImageSize(aspect) {
  return ["1:1", "3:4", "4:3", "9:16", "16:9"].includes(aspect) ? aspect : "1:1";
}
...
async function seedUserGeneration(uid, run_id, prompt, provider, metaExtra){
  ...
  try{
    const ug = `${SUPABASE_URL}/rest/v1/user_generations`;
    const meta = Object.assign({ source:'qwen-2', run_id, model:'qwen-2', status:'pending' }, (metaExtra||{}));
...
exports.handler = async (event) => {
  ...
  try{
    ...
    const model = isImageToImage ? "qwen2/image-edit" : "qwen2/text-to-image";
    ...
    if (isImageToImage) input.image_url = input_urls[0];
    ...
    const baseMeta = { source:"qwen-2", run_id, model, status:"processing", task_id: id, aspect_ratio, output_format:"png", mode: isImageToImage ? "image-to-image" : "text-to-image", refund_amount: cost, charge_cost: cost, model_cost: modelCost, subscription_unlimited: subscriptionUnlimited };
