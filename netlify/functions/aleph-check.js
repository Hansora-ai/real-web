...
// GET poller for Aleph (KIE) tasks. Finds mp4 and backfills Supabase.
// Mirrors vv-check.js but points to the Aleph "record-info" endpoint.

const VERSION_TAG = "aleph-check-GET-2025-10-01+v1";
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Query KIE (Aleph record detail)
    const url = `${KIE_BASE}/api/v1/aleph/record-info?taskId=${encodeURIComponent(taskId)}`;
      ...
      if (SUPABASE_URL && SERVICE_KEY) {
        ...
        if (idToPatch) {
          ...
          const ir = await fetch(UG_URL, {
            method: "POST",
            headers: { ...sb(), "Content-Type": "application/json", "Prefer": "return=minimal" },
            body: JSON.stringify({ user_id: uid || "00000000-0000-0000-0000-000000000000", provider:"aleph", kind:"video", prompt:null, result_url: video_url, meta: { run_id, task_id: taskId, status: "done" } })
          });
          patched = ir.ok;
