// netlify/functions/run-seedream-4-5.js
// Seedream 4.5 launcher with Kling 2.6-style server-side credit debit (idempotent per run_id).
// Client must NOT debit credits. Credits are charged only here using SUPABASE_SERVICE_ROLE_KEY.
//
// Env: KIE_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, SITE_BASE (optional)
//
// Models:
// - seedream/4.5-text-to-image (no image_input)
// - seedream/4.5-edit (with image_input)
//
...
const VERSION_TAG  = "seedream45_fn_kling26_style_v2";
...
async function seedUserGeneration(uid, run_id, prompt, metaExtra){
  ...
  try{
    const ug = `${SUPABASE_URL}/rest/v1/user_generations`;
    const meta = Object.assign({ source:'seedream-4.5', run_id, model:'seedream-4.5', status:'pending' }, (metaExtra||{}));
    const rIns = await fetch(ug, {
      method: 'POST',
      headers: { 'apikey': SERVICE_KEY, 'Authorization': `Bearer ${SERVICE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=representation' },
      body: JSON.stringify({ user_id: uid, provider: 'Seedream 4.5', kind: 'image', prompt, result_url: null, meta }),
    });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const image_input = urls.map(u => encodeURI(String(u)));
    const hasImages = image_input.length > 0;
    ...
    // Seed generation row
    const seeded = await seedUserGeneration(uid, run_id, prompt, { size, hasImages });
    ...
    if (!create.ok) {
      try {
        if (SUPABASE_URL && SERVICE_KEY && row_id) {
          await patchUserGenerationMetaById(row_id, { source:"seedream-4.5", run_id, model:"seedream-4.5", status:"create_failed", task_id: taskId, raw: js });
        }
      } catch {}
    ...
    try {
      if (SUPABASE_URL && SERVICE_KEY && row_id) {
        await patchUserGenerationMetaById(row_id, { source:"seedream-4.5", run_id, model:"seedream-4.5", status:"processing", task_id: taskId, size, hasImages });
      }
    } catch {}

    // Charge once per run_id (Kling 2.6 style)
    const baseMeta = { source:"seedream-4.5", run_id, model:"seedream-4.5", status:"processing", task_id: taskId, size, hasImages };
