...
// Same as your current MJ-aware callback, but if 2–4 image URLs are present
// it inserts ALL of them into nb_results (same user_id/run_id).
// No schema changes. Endpoint/inputs/outputs unchanged.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Collect up to 4 final URLs from payload (prefer) or verification
    let urls = pickResultUrls(data, 4);
    const haveAny = urls.some(isAllowedFinal);

    if ((!haveAny) && taskId && KIE_KEY) {
      try {
        const verified = await fetchMJorJobsAll(taskId, 4);
        if (verified.length) urls = verified;
      } catch {}
    }
    ...
    if (!finalUrls.length) {
      return reply(200, { ok:true, saved:false, note:'no allowed final image_url; not inserting', debug:{ taskId: !!taskId, status: statusStr || 'unknown' } });
    }

    // Update user_generations placeholder (mark done + store first URL)
    try {
    ...
    // Insert ALL images (1..4) into nb_results. Use merge-duplicates to avoid dup rows.
