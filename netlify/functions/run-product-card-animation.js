// netlify/functions/run-product-card-animation.js
// Product-card-only image-to-video runner. Keep separate from the generic Grok video page runner.
...
const CALLBACK_BASE = `${SITE_BASE}/.netlify/functions/product-card-check`;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const duration = 6;
    const checker = 'product-card-check';
    if (!KIE_KEY) return json(500, { ok: false, error: 'missing_kie_env' });
    ...
    const cost = costFor();
    const providerSource = 'kie';
    const source = 'product-card-animation';
    const diagnostic = requestDiagnostic(event, { ...body, image_urls: images }, duration);
    const metaBase = { source, provider_source: providerSource, engine: 'grok', run_id: runId, status: 'pending', refund_amount: cost, checker, diagnostic };
    ...
    const created = await createKieTask({ body: { ...body, image_urls: images }, prompt, duration, uid, runId });

    if (!created.res.ok) {
      const errorSummary = summarizeProviderError(created.data) || genericProviderFailure(providerSource, created.res.status);
      await patchGeneration(rowId, {
        ...metaBase,
        status: 'failed',
        error: created.data,
