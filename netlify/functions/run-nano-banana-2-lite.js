      ...
      try {
        if (SUPABASE_URL && SERVICE_KEY && row_id) {
          await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
            ...
            body: JSON.stringify({ meta: { source:"nano-banana-2-lite", run_id, model:"nano-banana-2-lite", status:"create_failed", task_id: taskId, raw: js, refund_amount: chargeCost, charge_cost: chargeCost, subscription_unlimited: subscriptionUnlimited } })
          });
        }
      } catch {}
      ...
      if (SUPABASE_URL && SERVICE_KEY && row_id) {
        await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
          ...
          headers: {
            ...
            body: JSON.stringify({ meta: { source:"nano-banana-2-lite", run_id, model:"nano-banana-2-lite", status:"processing", task_id: taskId, size, resolution, refund_amount: chargeCost, charge_cost: chargeCost, subscription_unlimited: subscriptionUnlimited } })
        });
      }
    } catch {}

    // Debit credits AFTER provider accepted and exactly once per (uid, run_id)
    const baseMeta = { source:"nano-banana-2-lite", run_id, model:"nano-banana-2-lite", status:"processing", task_id: taskId, size, resolution, refund_amount: chargeCost, charge_cost: chargeCost, subscription_unlimited: subscriptionUnlimited };
