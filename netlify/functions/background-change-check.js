// netlify/functions/background-change-check.js
// Dedicated checker/refunder for Background Change jobs.
...
exports.handler = async (event) => {
  try {
    ...
    const row = await findBackgroundChangeGeneration(ids);
    if (!row) return json(200, { ok: false, status: 'ignored', reason: 'not_background_change_processing' });
...
function isBackgroundChangeRow(row) {
  const meta = row?.meta && typeof row.meta === 'object' ? row.meta : {};
  const provider = String(row?.provider || '').toLowerCase();
  const engine = String(meta.engine || '').toLowerCase();
  const feature = String(meta.source_feature || '').toLowerCase();
  return feature === 'background-change' || engine === 'background-change-gemini-omni' || provider === 'background change';
}

async function findBackgroundChangeGeneration(ids) {
  ...
  for (const query of queries) {
    ...
    if (!row || row.result_url || !isBackgroundChangeRow(row)) continue;
...
function collectKnownInputUrls(row) {
  ...
  walk(meta.original_video_url, true);
  walk(meta.input_window, true);
