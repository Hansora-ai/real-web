...
const FACE_SWAP_PROMPT = `Use image 1 as the target/base photo and image 2 as the replacement character/identity reference, and create a perfectly realistic identity replacement where the person in image 1 is fully changed into the person from image 2 while keeping everything else from image 1 exactly the same, including the pose, body position, head angle, facial expression as much as possible, gaze direction, clothing, accessories, hairstyle continuity when needed, background, environment, framing, crop, composition, camera angle, lens perspective, lighting direction, shadow placement, exposure, color grading, depth of field, sharpness, grain, and overall image quality; preserve image 1 as the exact original shot in every non-identity aspect and use image 2 only to transfer the new identity with maximum accuracy, including facial structure, face shape, eyes, eyelids, eyebrows, nose, lips, cheekbones, jawline, chin, skin tone, age impression, and natural asymmetry, so the final result clearly looks like the person from image 2 and not a mix of both people; the replacement must be seamless and photorealistic with realistic skin pores, natural texture, fine detail, believable highlights and shadows, and perfect blending into the jawline, cheeks, forehead, ears, hairline, neck, and surrounding skin, with no seams, cutout effect, pasted look, mismatched tones, plastic skin, waxy skin, over-smoothed skin, fake AI texture, doll-like appearance, or beauty-filter look; do not change the outfit, body, hands, background, scene, lighting setup, composition, camera position, or any non-face element unless a tiny natural blend adjustment is absolutely necessary, and do not add extra people, extra faces, duplicated features, warped anatomy, broken eyes, distorted teeth, melted skin, mismatched hair, text, logos, or watermarks; the final output must be one clean high-end realistic image that looks like the exact same photo as image 1, but with the character fully and convincingly replaced by the identity from image 2.`;
