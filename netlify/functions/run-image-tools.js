...
const FACE_SWAP_PROMPT = `Use image 1 as the target/base photo and image 2 as the new character/identity reference, and create a perfect full character replacement where the person in image 1 is completely transformed into the person from image 2 while keeping the exact original shot from image 1; preserve image 1’s pose, body position, hand position, head angle, facial expression as much as possible, gaze direction, clothing, accessories, framing, crop, composition, camera angle, lens perspective, background, environment, lighting direction, shadow placement, exposure, color grading, depth of field, sharpness, grain, and overall image quality, but fully replace the visible character so the final person clearly becomes the person from image 2; transfer the identity and character appearance from image 2 completely, including the face, facial structure, face shape, eyes, eyebrows, nose, lips, jawline, chin, hair, hairstyle, hair color, hair texture, hairline, skin tone, visible skin appearance, and the overall recognizable look of the person from image 2; the hands must keep the same position and gesture from image 1, but their visible skin tone and character appearance should match the person from image 2 naturally; the final result must look like the exact same real photo from image 1, but as if the person originally photographed was the person from image 2; make the result extremely photorealistic and perfectly blended with realistic skin pores, natural skin texture, realistic hair strands, believable shading, natural highlights, accurate shadows, and seamless blending around the face, hairline, neck, ears, and hands, with no seams, no pasted look, no cutout effect, no mismatched skin, no mismatched hair, no plastic skin, no waxy skin, no over-smoothed skin, no fake AI texture, and no unrealistic anatomy; do not change the camera setup, scene, background, body pose, hand position, composition, lighting setup, or other non-character scene elements, and do not add extra people, extra limbs, duplicate features, text, logos, or watermarks; final output must be one clean, high-end, ultra-realistic character swap where image 1 remains the same shot in every scene aspect, but the person is fully and convincingly replaced by the character from image 2.`;
