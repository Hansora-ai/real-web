...
const VERSION_TAG  = "nb_fn_server_debit_0_5_v1";
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!create.ok) {
      return ok({ submitted: false, note: "create_failed", status: create.status, response: js, version: VERSION_TAG });
    }
...
// --- debit credits immediately on accepted submit (server-side, service role) ---
// IMPORTANT: Client must NOT debit credits. Only this function does.
try {
  const COST = 0.5;
  if (SUPABASE_URL && SERVICE_KEY && uid && uid !== 'anon') {
    ...
    const r0 = await fetch(profGet, {
      headers: { 'apikey': SERVICE_KEY, 'Authorization': `Bearer ${SERVICE_KEY}` }
    });
    const j0 = await r0.json();
    const c0 = (Array.isArray(j0) && j0[0] && Number(j0[0].credits)) || 0;

    if (c0 < COST) {
      return ok({ submitted: false, note: "not_enough_credits", credits: c0, cost: COST, version: VERSION_TAG });
    }
    ...
    await fetch(`${base}/rest/v1/profiles?user_id=eq.${encodeURIComponent(uid)}`, {
      ...
      body: JSON.stringify({ credits: Number((c0 - COST).toFixed(1)) })
    });
  }
} catch (e) {
  console.warn('[nb] debit credits failed', e);
  // Do not fail the request; the UI will keep waiting and admin logs can reconcile if needed.
}
