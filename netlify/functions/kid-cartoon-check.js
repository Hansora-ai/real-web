// netlify/functions/kid-cartoon-check.js
// KIE checker/refunder for Kid Cartoon Studio generations only.
...
exports.handler = async (event) => {
  try {
    ...
    if (!isKidCartoonRow(row)) return json(200, { ok: false, status: "ignored", reason: "not_kid_cartoon" });
...
function isKidCartoonRow(row) {
  ...
  const engine = String(row?.meta?.engine || "").toLowerCase();
  return provider === "kid cartoon studio" || engine === "kid-cartoon-seedance-2.0";
}
