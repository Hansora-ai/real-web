// netlify/functions/run-seedance-cheap.js
// Text-only Seedance 2.0 route through BytePlus ModelArk.
// Any request with uploaded media must stay on run-seedance-2.js.
...
async function insertGeneration(uid, runId, prompt, meta) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/user_generations`, {
    ...
    body: JSON.stringify({ user_id: uid, provider: 'Seedance 2.0', kind: 'video', prompt, result_url: null, meta }),
  });
  const rows = await res.json().catch(() => []);
...
exports.handler = async (event) => {
  ...
  try {
    if (!ARK_KEY || !SUPABASE_URL || !SERVICE_KEY) return json(500, { ok: false, error: 'missing_env' });
