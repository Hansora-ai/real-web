...
exports.handler = async (event) => {
  ...
  try{
    ...
    const seeded = await seedUserGeneration(uid, run_id, prompt, provider, { aspect_ratio, tier, mode: isImageToImage ? "image-to-image" : "text-to-image", refund_amount: cost });
      ...
      try {
        if (SUPABASE_URL && SERVICE_KEY && row_id) {
          await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
            ...
            body: JSON.stringify({ meta: { source:"wan-2-7-image", run_id, model, status:"create_failed", task_id: id, raw: js, refund_amount: cost } })
          });
        }
      } catch {}
    ...
    try {
      if (SUPABASE_URL && SERVICE_KEY && row_id) {
        await fetch(`${SUPABASE_URL}/rest/v1/user_generations?id=eq.${encodeURIComponent(row_id)}`, {
          ...
          body: JSON.stringify({ meta: { source:"wan-2-7-image", run_id, model, status:"processing", task_id: id, aspect_ratio, tier, resolution:"1K", mode: isImageToImage ? "image-to-image" : "text-to-image", refund_amount: cost } })
        });
      }
    } catch {}

    // Debit credits AFTER provider accepted and exactly once per (uid, run_id)
    const baseMeta = { source:"wan-2-7-image", run_id, model, status:"processing", task_id: id, aspect_ratio, tier, resolution:"1K", mode: isImageToImage ? "image-to-image" : "text-to-image", refund_amount: cost };
